...
class CVAEM3(tf.keras.models.Model):
    def __init__(self, M1, n_labels = 10):
        super(CVAEM3, self).__init__()
        self.M1 = M1
        self.n_labels = n_labels

        self.z_Proj = tf.keras.layers.Dense(self.M1.latent_dim,
                                            activation='linear'
                                            )
        self.AuxClassifier = tf.keras.layers.Dense(self.n_labels,
                                                   activation='softmax'
                                                   )
        self.Concat = tf.keras.layers.Dense(self.M1.latent_dim,
                                            activation='linear'
                                            )

    def compile(self, optimizer):
        super(CVAEM3, self).compile()
        self.optimizer = optimizer

    @tf.function
    def train_step(self, X):
        with tf.GradientTape() as tape:
            mean, logvar = self.M1.Encoder(X)
            latent = self.M1.reparameteriation(mean, logvar)
            z = self.z_Proj(latent)
            y = self.AuxClassifier(latent)
            latent_ = self.Concat(tf.concat([z, y], axis=-1))
            reconstuction = self.M1.Decoder(latent_)
            reconstruction_loss = tf.reduce_mean(
                tf.keras.losses.binary_crossentropy(X, reconstuction)
            ) * 28 * 28
            kl_loss = tf.reduce_mean(
                -.5 * tf.reduce_sum((1 + logvar - tf.square(mean) - tf.exp(logvar + 1e-8)), axis=1))
            ELBO = reconstruction_loss + kl_loss
        grads = tape.gradient(ELBO, self.M1.Encoder.trainable_variables +
                              self.z_Proj.trainable_variables +
                              self.AuxClassifier.trainable_variables +
                              self.Concat.trainable_variables +
                              self.M1.Decoder.trainable_variables)
        self.optimizer.apply_gradients(
            zip(grads, self.M1.Encoder.trainable_variables +
                self.z_Proj.trainable_variables +
                self.AuxClassifier.trainable_variables +
                self.Concat.trainable_variables +
                self.M1.Decoder.trainable_variables)
